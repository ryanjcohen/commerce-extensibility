...
When a customer signs in and adds a new address, the address must be validated. Before the new address is saved in Commerce, a third-party address validator must run on the input information. If the address is not valid, an error message must be displayed.
...
When an admin updates or creates a product, a third-party system is used to validate the product attributes. For example, a third-party system can validate the product name.
...
The following `observer.catalog_product_save_after` payload was obtained from the code execution in the application. Some data has been adjusted or deleted for brevity.
...
async function main (params) {
  ...
  try {
    ...
    // If it is, the request is considered invalid.
    const response = {statusCode: 200}
    // set this variable to true if a success operation should be returned
...
Similarly, stock validation could be performed when adding a product to a quote using a `observer.sales_quote_add_item` webhook:

**Default payload:**

The following `observer.sales_quote_add_item` payload was obtained from the code execution in the application. The majority of values in the `product` section were deleted for brevity.
...
When a shopper goes to place an order, a third-party system is used to confirm that the items added to the order can be shipped to the selected address.
...
async function main (params) {
  ...
  try {
    ...
    const response = {statusCode: 200}
    params.order.addresses.forEach((address) => {
      if (address.address_type === 'shipping' && address.postcode > 70000) {
        response.body = JSON.stringify({
            op: "exception",
            message: `App Builder Webhook Response: Products cannot be shipped to postcode "${address.postcode}"`
        })
      }
    })
...
The following payload was obtained from the code execution in the application using a webhook type of `after`. The `address` section was deleted for brevity. 
...
When a shopper is checking out and enters a gift card code to check its balance, a third-party system is used to get the gift card's balance.
...
<method name="plugin.magento.gift_card_account.api.gift_card_account_management.check_gift_card" type="after">
    <hooks>
        <batch name="check_gift_card">
            <hook name="get_balance" url="{env:APP_BUILDER_URL}/get-gift-card-balance" method="POST" timeout="5000" softTimeout="1000">
                <headers>
                    <header name="x-gw-ims-org-id">{env:APP_BUILDER_IMS_ORG_ID}</header>
                    <header name="Authorization">Bearer {env:APP_BUILDER_AUTH_TOKEN}</header>
                </headers>
